...
export const formSchema = z.object({
	...
	pronunciation: z
		...
		.optional(),
	examples: z.array(textPairLargeSchema).max(5).optional().default([]),
	collocations: z.array(textPairLargeSchema).max(5).optional().default([]),
	derivations: z.array(textPairSchema).max(5).optional().default([]),
	phrasal_verbs: z.array(textPairSchema).max(5).optional().default([]),
	synonyms: z.array(textPairSchema).max(5).optional().default([]),
	antonyms: z.array(textPairSchema).max(5).optional().default([]),
	types: z.array(z.number()).optional().default([]),
	etymology: z
		.string()
		.max(500, {
			message: "語源は500文字以内で入力してください",
		})
		.optional(),
	note: z
		.string()
